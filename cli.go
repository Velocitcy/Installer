...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Velocity")
	var updateFlag = flag.Bool("repair", false, "Repair Velocity")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Velocity")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Velocity Installer' option to update, or run with --update-self")
			}
		}()
		...
		case "Quit":
			return
		case "Update Velocity Installer":
			if err := UpdateSelf(); err != nil {
				Log.Error("Failed to update self:", err)
				exitFailure()
			}
	...
	if install {
		...
		Log.Info("Downloading latest Velocity files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Velocity will likely not work.")
}

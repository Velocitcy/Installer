...
var BaseDir string
var VelocityDirectory string

func init() {
	if dir := os.Getenv("VELOCITY_USER_DATA_DIR"); dir != "" {
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, VelocityDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", VelocityDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+VelocityDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + VelocityDirectory + ": " + err.Error())
		}
	}
	return nil
}
